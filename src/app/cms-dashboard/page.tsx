...
const DashboardPage: React.FC = () => {
  ...
  const [data, setData] = useState<any>(null); // Ganti dengan tipe data yang sesuai jika diperlukan
  const [error, setError] = useState<string | null>(null);
  const router = useRouter();
  const [chartPeriod, setChartPeriod] = useState('Daily');

  console.log('User:', user); // Log user
  console.log('Loading:', loading); // Log status loading
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const res = await axios.get('/auth/me'); // Ganti dengan endpoint yang sesuai
        console.log('Fetched Data:', res.data); // Log respons API
